    ...
    Self: PoissonIterImpl<N>,
{
    ...
    fn initial_sample(&mut self) -> Point<N> {
        let p = self.poisson.dims.map(|x| self.rng.random_range(0.0..=x));
        self.add_point(&p, None);
        p
    }

    fn sample(&mut self) -> Option<Point<N>> {
        let active_idx = self.rng.random_range(0..self.active.len());
        let sample = &self.active[active_idx];
        for _ in 0..self.poisson.attempts {
            let cand = S::sample_nbhd(sample, &self.samples, &self.poisson, &mut self.rng);
            if self.is_sample_valid(&cand) {
                self.add_point(&cand, Some(sample.idx));
                return Some(cand);
            }
        }
...
impl<S: NbhdSampler<2>, R: Rng + SeedableRng> PoissonIterImpl<2> for PoissonIter<2, S, R> {
    #[inline(always)]
...
impl<S: NbhdSampler<3>, R: Rng + SeedableRng> PoissonIterImpl<3> for PoissonIter<3, S, R> {
    #[inline(always)]
