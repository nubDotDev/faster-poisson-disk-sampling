use super::Point;
use crate::iter::{ActiveSample, PoissonIterInner};
    ...
    fn sample_nbhd<S: NbhdSampler<2>, R: Rng + SeedableRng>(
        ...
        return from_fn(move || {
            let s = 2.0
                ...
                * rng
                    .random_range(0.5f64.powf(inner.poisson.cdf_exp)..=1.0)
...
impl NbhdSampler<2> for ParentalNbhdSampler<2> {
    fn sample_nbhd<S: NbhdSampler<2>, R: Rng + SeedableRng>(
        ...
        let theta_range = match sample.parent_idx {
            ...
            Some(parent_idx) => {
                let parent = &inner.samples[parent_idx];
                let d = [
                    (parent[0] - p[0]) / inner.poisson.radius,
                    (parent[1] - p[1]) / inner.poisson.radius,
                ];
